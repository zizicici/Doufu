...
10. 多任务执行时，每个子任务改动落盘后要刷新文件候选，避免后续任务读取旧上下文。
11. 取消请求必须透传到规划/检索/生成全链路，不允许在 fallback 中吞掉 `CancellationError`。
12. 若后端在 `200 + SSE` 阶段返回 `response.failed`，仍需走与 4xx 一致的降级重试策略。
